...
- Process files in order: Parse ÔåÆ Extract All ÔåÆ SKU Lookup ÔåÆ AI Classify ÔåÆ Store ÔåÆ Metadata
...
class BatchProcessor:
    ...
    - File management (inbox ÔåÆ processed/failed)
    ...
    def process_single_file(self, xml_path: str) -> bool:
        ...
        try:
            ...
            # 6. Create purchase_details entries for Google Sheets export
            logger.info(f"[PURCHASE_DETAILS] Creating purchase details")
    ...
    def _create_invoice_metadata(self, invoice_id: int, parsed_data: Dict[str, Any], classifications: List[Dict[str, Any]]) -> None:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                is_immediate = payment_terms == 'PUE'     # Pago en Una Sola Exhibici├│n
    ...
    def _check_duplicate_invoice(self, uuid: str, filename: str) -> bool:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                if existing_invoice:
                    logger.warning(f"­ƒöä DUPLICATE DETECTED: Invoice UUID {uuid} already exists (ID: {existing_invoice.id})")
    ...
    def _handle_duplicate_invoice(self, uuid: str, filename: str, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.warning(f"­ƒôè DUPLICATE INVOICE DETAILS:")
    ...
    def _save_to_purchase_details(self, invoice_id: int, parsed_data: Dict[str, Any], classifications: List[Dict[str, Any]]) -> None:
        """
        Create purchase_details entries for Google Sheets export.
        ...
        try:
            with self.db_manager.get_session() as session:
                from src.data.models import PurchaseDetails
                ...
                # Calculate MXN total with business logic
                original_total = self._safe_decimal(parsed_data.get('total_amount', 0))
                if currency == 'MXN':
                    invoice_mxn_total = original_total
                    exchange_rate = Decimal('1.0')
                else:
                    invoice_mxn_total = original_total * exchange_rate
                
                # Payment terms business logic
                is_installments = payment_terms == 'PPD'
                is_immediate = payment_terms == 'PUE'
                
                # Create purchase_details entry for each item
                for idx, (item_data, classification) in enumerate(zip(parsed_data.get('items', []), classifications)):
                    # Calculate item-level MXN values
                    item_total = self._safe_decimal(item_data.get('total_amount', 0))
                    item_mxn_total = item_total * exchange_rate if currency != 'MXN' else item_total
                    
                    unit_price = self._safe_decimal(item_data.get('unit_price', 0))
                    unit_mxn_price = unit_price * exchange_rate if currency != 'MXN' else unit_price
                    ...
                    conversion_factor = self._safe_decimal(classification.get('conversion_factor', 1))
                    standardized_quantity = original_quantity * units_per_package
                    standardized_mxn_value = standardized_quantity * unit_mxn_price if standardized_quantity else None
                    ...
                    purchase_detail = PurchaseDetails(
                        ...
                        # Business Logic
                        invoice_mxn_total=invoice_mxn_total,
                        is_installments=is_installments,
                        is_immediate=is_immediate,
                        ...
                        total_tax_amount=self._safe_decimal(item_data.get('total_tax_amount', 0)),
                        ...
                        item_mxn_total=item_mxn_total,
                        standardized_mxn_value=standardized_mxn_value,
                        unit_mxn_price=unit_mxn_price
                    )
                    
                    session.add(purchase_detail)
                
                logger.info(f"Created {len(classifications)} purchase_details entries for invoice {invoice_uuid}")
                
        except Exception as e:
            logger.error(f"Failed to create purchase_details: {e}")

# Note: The purchase_details table provides a flattened, denormalized view
# optimized for Google Sheets exports and business intelligence queries. 
